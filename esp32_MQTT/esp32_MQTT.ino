...
const char* board_num = "esp32_002";//board number, If use multiple board, set the name differently ex)"esp32_002"
const char* board_num_topic = "esp32_002/output";//Node-RED -> esp32 board Topic
const char* board_num_O3 = "esp32_002/O3";//esp32 -> Node-RED Topic
...
void loop() {
  ...
  if (now - lastMsg > 5000) {//Time period setting
    ...
    if (messageO3 == "on") {//if received message "on"
      ...
      char O3String[8];
      dtostrf(O3, 1, 2, O3String);//float -> char array so that it can be sent
      lcd.setCursor(0, 0); lcd.print("AP:");
      lcd.setCursor(3, 0); lcd.print(IP);
      lcd.setCursor(0, 1); lcd.print("O3:");
      lcd.setCursor(3, 1); lcd.print(O3String);
      client.publish(board_num_O3, O3String);//Topic to send, value
    }
